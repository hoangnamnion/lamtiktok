...
// Fixed Default State (Tối ưu chuẩn 100% theo ảnh mẫu)
const state = {
...
// Render Full HD Canvas (1170 x 1560) for Export
function renderExportCanvas() {
  ...
  // 5. Cấu hình Typography cho Canvas - Căn lề thụt sang phải & không gian chữ nhỏ gọn chuẩn ảnh mẫu
  ...
  const paddingLeft = 210;
  const paddingRight = 175;
  const maxLineWidth = canvasWidth - paddingLeft - paddingRight;

  exportCtx.font = `300 ${fontSize}px ${fontFamily}`;
  ...
  const authorTagHeight = textLineCount > 0 ? 45 : 0;
  ...
  for (const line of wrappedLines) {
    ...
    if (state.textAlign === 'center') {
      currentX = paddingLeft + (maxLineWidth - lineWidth) / 2;
    } else if (state.textAlign === 'right') {
      currentX = paddingLeft + maxLineWidth - lineWidth;
    }
  ...
  // 9. Vẽ Chữ ký tác giả #nam_26th4 dưới dòng cuối cùng (Căn phải khối văn bản)
  if (textLineCount > 0 && state.authorTag) {
    currentY += 18;
    exportCtx.font = `400 28px ${fontFamily}`;
    exportCtx.fillStyle = '#8e8e99';
    const tagWidth = exportCtx.measureText(state.authorTag).width;
    const tagX = paddingLeft + maxLineWidth - tagWidth - 10;
